...
HP_MULTIPLIERS_RING = HP_MULTIPLIERS_SECTOR * 6
...
def build_model():
    ...
    # -----------------------------------------------------
    # 4. Connect buffers, three inlets, three outlets, and ring closure
    # -----------------------------------------------------
    ...
    for idx, ring_node_idx in enumerate(INLET_NODE_INDICES):
        ...
        hot_leg_to_ring.append(
            MacroFlowJunction(
                name=f"J_HotLeg_{idx + 1}_RingNode_{ring_node_idx}",
    ...
    for idx, ring_node_idx in enumerate(OUTLET_NODE_INDICES):
        ring_to_manifold.append(
            MacroFlowJunction(
                name=f"J_RingNode_{ring_node_idx}_Manifold_{idx + 1}",
                from_vol=ring_channel.volumes[ring_node_idx],
                to_vol=manifolds[idx].volumes[0],
                macro_vol=manifolds[idx].volumes[0],
                multiplier=2,
                flow_area=AREA_MANIFOLD,
                k_loss=0.0,
            )
...
def run_case(
    ...
    while sys_mgr.global_time < t_end:
        ...
        for idx, junc in enumerate(ring_to_manifold, start=1):
            ...
            row[f"W_ring_to_manifold_macro_{idx}"] = float(
                junc.get_mass_flow_for(manifolds[idx - 1].volumes[0])
            )
...
def print_model_summary(model):
    print("Model: one full-ring RingHP with 3 inlets and 3 outlets")
...
if __name__ == "__main__":
    run_case(
        ...
        t_end=DEFAULT_T_END,
        print_every_time=DEFAULT_PRINT_EVERY_TIME,
        csv_path=os.path.join(current_dir, "collector_ring_full_ringhp_buffered_50s_history.csv"),
        restart_save_path=os.path.join(current_dir, "collector_ring_full_ringhp_buffered_50s_restart.npz"),
        restart_save_every=DEFAULT_RESTART_SAVE_EVERY,
        restart_from=None,
    )
