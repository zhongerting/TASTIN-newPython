import os
import sys

if hasattr(sys.stdout, "reconfigure"):
    sys.stdout.reconfigure(encoding="utf-8", line_buffering=True, write_through=True)
if hasattr(sys.stderr, "reconfigure"):
    sys.stderr.reconfigure(encoding="utf-8", line_buffering=True, write_through=True)

current_dir = os.path.dirname(os.path.abspath(__file__))
root_dir = os.path.abspath(os.path.join(current_dir, ".."))
if root_dir not in sys.path:
    sys.path.insert(0, root_dir)

import CoolantLoop.model_collector_ring_full_ringhp as model


def main():
    model.run_case(
        case_name="collector_ring_full_ringhp_buffered_half_ringflow_200s_resume_from60s",
        t_end=200.0,
        print_every_time=1.0,
        csv_path=os.path.join(
            current_dir,
            "collector_ring_full_ringhp_buffered_half_ringflow_200s_resume_from60s_history.csv",
        ),
        restart_from=os.path.join(
            current_dir,
            "collector_ring_full_ringhp_buffered_200s_resume_from50s_restart_t0060s.npz",
        ),
        restart_save_path=os.path.join(
            current_dir,
            "collector_ring_full_ringhp_buffered_half_ringflow_200s_resume_from60s_restart.npz",
        ),
        restart_save_every=20.0,
    )


if __name__ == "__main__":
    main()
