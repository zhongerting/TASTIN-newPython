...
AREA_HOT_LEG = 5.9798e-4
DH_HOT_LEG = 0.0276
L_HOT_LEG = 2.19632
N_HOT_LEG = 8

AREA_MANIFOLD = 2.5434e-4
DH_MANIFOLD = 0.018
L_MANIFOLD = 0.40911
N_MANIFOLD = 5
MANIFOLD_LENGTHS = [0.40911, 1.41912, 1.41912]
MANIFOLD_NODE_COUNTS = [5, 17, 17]
...
DEFAULT_CASE_NAME = "collector_ring_6segment_v9_interface_200s"
DEFAULT_T_END = 200.0
DEFAULT_PRINT_EVERY_TIME = 20.0
DEFAULT_RESTART_SAVE_EVERY = 20.0
...
def build_model():
    ...
    manifolds = [
        IncompressibleFluidChannel(
            ...
            n_nodes=MANIFOLD_NODE_COUNTS[idx - 1],
            total_length=MANIFOLD_LENGTHS[idx - 1],
    ...
    for idx, key in enumerate(INLET_MIX_KEYS):
        inlet_buffer_to_hot_leg.append(
            MacroFlowJunction(
                name=f"J_InletBuffer_HotOutletBranch_{idx + 1}",
        ...
        hot_leg_to_inlet_mix.append(
            FlowJunction(
                name=f"J_HotOutletBranch_{idx + 1}_InletMix_{key}",
...
def print_model_summary(model):
    print("Model: 6 RingHP segments + V9-interface inlet/outlet branches")
    ...
    print(f"  V9 interface inlet T: {T_INLET:.6f} K")
    print(
        "  HotOutletBranch     : "
        f"L={L_HOT_LEG:.5f} m, A={AREA_HOT_LEG:.7e} m2, Dh={DH_HOT_LEG:.5f} m, "
        f"nodes={N_HOT_LEG}"
    )
    print(
        "  Manifold lengths    : "
        + ", ".join(f"{length:.5f} m" for length in MANIFOLD_LENGTHS)
    )
    print(
        "  Manifold nodes      : "
        + ", ".join(str(nodes) for nodes in MANIFOLD_NODE_COUNTS)
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the 6-segment collector-ring model with V9 interface branch geometry."
    )
    ...
    parser.add_argument("--print-every-time", type=float, default=DEFAULT_PRINT_EVERY_TIME)
    parser.add_argument("--restart-save-every", type=float, default=DEFAULT_RESTART_SAVE_EVERY)
...
if __name__ == "__main__":
    args = parse_args()
    csv_path = args.csv_path or os.path.join(current_dir, f"{args.case_name}_history.csv")
    restart_save_path = args.restart_save_path or os.path.join(
        current_dir, f"{args.case_name}_restart.npz"
    )
    run_case(
