...
- The mixing volumes are not heat-pipe-bearing ring nodes. Inlet mixing volumes use `AREA_HOT_LEG * (2*DH_HOT_LEG)` and outlet mixing volumes use `AREA_MANIFOLD * (2*DH_MANIFOLD)`. Links from mixing volumes into/out of RingHP segments use `AREA_RING`.
- Local losses are explicitly assigned in `model_collector_ring_6segment_geometry100hp_potassium_mixed.py`: inlet buffer to hot leg `K=1.5`, hot leg to inlet mix `K=1.0`, inlet mix to ring segment `K=0.5`, ring segment to outlet mix `K=0.7`, outlet mix to manifold `K=1.0`, and manifold to outlet buffer `K=1.1`. These are first-round engineering values from `液态金属冷却剂集流环局部阻力系数调研.md`; each value uses the velocity from that junction's configured `flow_area`.
- RingHP internal tube-bank pressure drop remains handled by the existing dynamic loss parameters. The ring-segment outlet link keeps `RingHP.outlet_dynamic_loss_params` and adds only the outlet-mixing local-loss `K=0.7`, so tube-bank loss is not replaced or double-counted as a fixed K.
- The history CSV records pressure diagnostics for hot legs, manifolds, sector endpoints, and actual-flow-direction pressure drops (`dP_*`) on macro branch links, ring entry links, and ring exit links. Use these columns when separating ring-internal pressure drop from inlet/outlet local losses.
- A 2026-06-08 short restart check with local losses used the prefix `collector_ring_6segment_geometry100hp_potassium_mixed_local_loss_510s_from500s` from the `500 s` restart to `510 s`. It completed without warning; final `T_out_avg=756.514488 K`, inlet-outlet `dT=86.484517 K`, `W_ring_in_total=0.650000 kg/s`, and `W_ring_out_total=0.650007 kg/s`. Compared with the no-local-loss LSODA-wall `510 s` baseline, `T_out_avg` changed by only about `2.6e-5 K`. Final pressure diagnostics showed inlet-buffer outlet to outlet-buffer outlet pressure difference about `1.62 kPa`; outlet mix to manifold averaged about `507 Pa`, and manifold to outlet buffer averaged about `286 Pa`, confirming that outlet-side small-area connections dominate the added local pressure drop.
...
- As of 2026-06-08, this case configures each ring-wall sector solid with `RING_WALL_ODE_METHOD="LSODA"` through `BaseHeatConduction.set_ode_method()`. Each representative heat pipe uses anisotropic wick conductivity, uncapped axial wick conductivity, `HeatPipe2D.time_integrator="theta_implicit"`, `theta_implicit_value=0.6`, and `set_implicit_boundary_linearization(True)`. The generic heat-conduction BDF path is not used for these heat-pipe solids.
...
- A follow-up run with the same heat-pipe settings and `RING_WALL_ODE_METHOD="LSODA"` used the prefix `collector_ring_6segment_geometry100hp_potassium_mixed_lsoda_wall_700s_from500s`. It also completed to `700 s` without RuntimeWarning; final `T_out_avg=756.533151 K`, inlet-outlet `dT=86.465854 K`, and the result matched the previous BDF-wall continuation to sub-microkelvin scale.
...
## 13. 2026-06-07 BDF warning sensitivity matrix

- `run_collector_ring_warning_sensitivity_matrix.py` runs the 6-segment potassium case through a two-stage warning test: `500 s -> 510 s` transition with warnings recorded, followed by `510 s -> 530 s` validation with `RuntimeWarning` promoted to an exception.
- The matrix cases are: A isotropic wick with `k_cap=1.2e6 W/(m*K)`, B anisotropic wick with uncapped axial conductivity, C anisotropic wick with `k_cap=1.2e6 W/(m*K)`, D anisotropic wick with `k_cap=1.2e6 W/(m*K)` plus `enable_frozen_property_correction=True`, E anisotropic wick with `k_cap=1.2e6 W/(m*K)` plus the HeatPipe2D `implicit_euler` integrator, F anisotropic wick with uncapped axial conductivity plus `implicit_euler`, G anisotropic wick with `k_cap=1.2e6 W/(m*K)` plus `theta_implicit(theta=0.6)`, and H anisotropic wick with uncapped axial conductivity plus `theta_implicit(theta=0.6)`.
- On the first validation warning, the script writes `warning_test_<case>_530s_first_warning.txt` with the triggering solid, step size, temperature range, conductance ranges, wick temperatures, and axial/radial wick conductivity diagnostics.
- `HeatPipe2D.time_integrator="implicit_euler"` is a heat-pipe-only backward-Euler conduction step. It treats internal conductance implicitly and freezes boundary/source terms within each Picard property iteration. A 2026-06-07 E-case run completed `500->510 s`, `510->530 s` warning-as-error, and `530->600 s` warning-as-error without RuntimeWarning; at `600 s`, `T_out_avg≈757.577 K`, inlet-outlet `dT≈85.422 K`, and maximum representative heat-pipe internal `dT≈10.08 K`.
- A 2026-06-08 F-case run with uncapped axial wick conductivity also completed `500->510 s`, `510->530 s` warning-as-error, and `530->600 s` warning-as-error without RuntimeWarning. At `600 s`, `T_out_avg≈756.538 K`, inlet-outlet `dT≈86.461 K`, maximum sampled axial wick conductivity was `≈3.80e7 W/(m*K)`, and maximum representative heat-pipe internal `dT≈4.78 K`.
- `HeatPipe2D.time_integrator="theta_implicit"` is a heat-pipe-only theta-method option. The default test value is `theta_implicit_value=0.6`; `theta=1.0` is equivalent to the existing `implicit_euler` branch.
- 2026-06-08 G/H theta-method checks completed `500->510 s`, `510->530 s` warning-as-error, and `530->600 s` warning-as-error without RuntimeWarning. H used uncapped axial wick conductivity; at `600 s`, `T_out_avg≈756.538 K`, inlet-outlet `dT≈86.461 K`, maximum sampled axial wick conductivity was `≈3.80e7 W/(m*K)`, and maximum representative heat-pipe internal `dT≈4.78 K`.

## 14. 2026-06-08 heat-pipe implicit radiation boundary option

- `HeatPipe2D.set_implicit_boundary_linearization(True)` is a heat-pipe-only option for the sparse `implicit_euler` and `theta_implicit` paths. The default remains `False`; the BDF path and generic `BoundaryRegion` behavior are unchanged.
- When enabled, resistance-type heat-pipe boundaries are assembled as `Q = G * (T_eff - T_node)` in the heat-pipe matrix. This includes bare `DynamicRadiationResistanceBC` on the adiabatic/condenser outer wall and the `HPwithFin` equivalent fin branch `ResistanceBC`. Flux-only boundary terms remain explicit RHS terms.
- `run_collector_ring_warning_sensitivity_matrix.py` adds I/J cases for this option: I is uncapped anisotropic `theta_implicit(theta=0.6)` with implicit boundary linearization, and J is uncapped anisotropic `implicit_euler` with implicit boundary linearization. Compare I against H and J against F from the same restart before treating the option as a production setting.
- 2026-06-08 checks: I completed `500->510 s` with warnings recorded and `510->530 s` warning-as-error without RuntimeWarning; at `530 s`, `T_out_avg=756.536614 K` and inlet-outlet `dT=86.462391 K`. J completed the same windows without RuntimeWarning; at `530 s`, `T_out_avg=756.536590 K` and `dT=86.462416 K`.

## 15. 2026-06-08 closed-loop 6segment potassium case

- `model_collector_ring_6segment_geometry100hp_potassium_closed_loop.py` is the first closed-loop version of the 6-segment potassium collector-ring case. It keeps the segmented RingHP layout, heat-pipe solver settings, local-loss K values, and pressure diagnostics from the open-loop mixed-node case, but replaces `inlet_boundary`, `outlet_boundary`, and `InletJunction` with a `PressurizerVolume`, `PumpJunction`, `HeaterChannel`, and `ReturnChannel`.
- The pressure reference is the pressurizer at `P_REF=160000 Pa`, matching the current open-loop `P_OUTLET`. The initial pump estimate was based on the open-loop local-loss pressure drop of about `1.622 kPa`; a `Q_HEATER=0` pump scan to `1 s` gave `W_total_loop=1.276 kg/s` at `2000 Pa` and `1.344 kg/s` at `2200 Pa`, so the closed-loop v1 default pump head is set to `2070 Pa`.
- The heater is applied as a persistent fluid source on `HeaterChannel` through `SystemManager.add_persistent_fluid_source()`, so it is restored after the solver clears transient fluid sources. The default heater power is `98 kW`, from the open-loop quasi-steady enthalpy estimate `1.3 kg/s * (h(843 K)-h(756.54 K))`.
- The default initialization combines an analytic closed-loop fluid profile with open-loop thermal-state migration. Fluid starts with heater inlet/return/outlet side at `756.5366 K` and heater outlet/inlet side at `843 K`; RingHP wall and representative heat-pipe solids are loaded from `collector_ring_6segment_geometry100hp_potassium_mixed_local_loss_630s_from510s_restart.npz`. This loads 24 solid states and avoids the earlier uniform-843 K closed-loop startup, which artificially raised the whole loop temperature.
- A smoke run `collector_ring_closed_loop_smoke_0p1s` with `Q_HEATER=0` completed without pressure-matrix issues. The run loop includes a `dt <= 1e-12` guard to avoid taking a zero-length LSODA step at the exact target time.
- A corrected short heated run with `pump_delta_p=2070 Pa` and `Q_HEATER=98 kW` completed to `30 s` using the prefixes `collector_ring_closed_loop_thermalinit_heater98kw_10s` and `collector_ring_closed_loop_thermalinit_heater98kw_30s_from10s`. At `30 s`, `W_total_loop=1.29847 kg/s`, `W_ring_in_total=0.64929 kg/s`, `W_ring_out_total=0.64923 kg/s`, `T_inlet_buffer_out=846.793 K`, `T_out_avg=760.260 K`, `Q_reject_est=97.947 kW`, and `Q_balance_residual=0.134 kW`.
- The corrected `30 s` heated result is close to the open-loop temperature split but still about `3.8 K` warmer in absolute level. This is now a heater-power/closed-loop-equilibration issue rather than the previous initialization error. The next validation step should continue this corrected restart toward `100 s`; if the absolute loop temperature remains high, reduce `Q_HEATER` slightly rather than changing the heat-pipe radiation model.
