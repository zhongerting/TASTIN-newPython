...
- A short heated run with `pump_delta_p=2070 Pa` and `Q_HEATER=98 kW` completed to `30 s` using the prefixes `collector_ring_closed_loop_heater98kw_10s` and `collector_ring_closed_loop_heater98kw_30s_from10s`. At `30 s`, `W_total_loop=1.29618 kg/s`, `W_ring_in_total=0.64765 kg/s`, `W_ring_out_total=0.64814 kg/s`, `T_inlet_buffer_out=892.436 K`, `T_out_avg=804.567 K`, `Q_reject_est=99.274 kW`, and `Q_balance_residual=3.184 kW`.
- The `30 s` heated result is still a thermal transient from the uniform initial condition, not a steady result. The next validation step should continue this closed-loop restart toward `100 s` and then `500-700 s`, and tune `Q_HEATER` only after the loop approaches quasi-steady behavior.
