"""Run the 350 K V14 shield/radiator startup segment with restart support."""
...
def build_case(initial_temperature_k: float = 350.0, orbit_phase_origin_s: float = 0.0):
    build = build_v14_case_a_system(
        ...
        flow_config=FullLoopFlowConfig(total_flow_kg_s=0.26),
        pump_config=FullLoopPumpConfig(
            pump_total_head_pa=8516.44886986068,
...
def run(
    ...
    if restart_in is None:
        _set_uniform_temperature(build, 350.0)
    ...
    end_time = float(system.global_time) + float(duration_s)
    ...
    while system.global_time < end_time - 1.0e-10:
        ...
        dt = min(float(dt), float(max_dt_s), end_time - system.global_time)
