...
from testModule.Full_Loop_Cases.Full_Loop_Cases_10kW import (  # noqa: E402
    FullLoopCoreConfig,
    FullLoopFlowConfig,
    FullLoopPumpConfig,
...
class DebugRunConfig:
    ...
    external_heat_period_s: float = 5668.144369
...
def build_debug_case(config: DebugRunConfig, *, apply_fixed_power: bool = True) -> Dict[str, Any]:
    build = build_v14_case_a_system(
        ...
        radiator_config=V14HeatPipeRadiatorConfig(
            ...
            external_heat_period_s=float(config.external_heat_period_s),
            external_heat_time_origin_s=float(config.external_heat_time_origin_s),
        ),
    )
    system = build["system"]
...
def _write_latest_state(out_dir: Path, stage: int, restart_path: Path, summary_path: Optional[Path], metrics: Dict[str, Any], case_name: str = CASE_NAME) -> None:
...
def run_debug_case(config: DebugRunConfig) -> Dict[str, Any]:
    case_name = str(config.case_prefix)
    ...
    for stage_index, duration_s in enumerate(config.stage_durations_s, start=1):
        ...
        while float(system.global_time) < stage_end - 1.0e-12:
            ...
            if _low_temperature_triggered(config, row_for_checks):
                ...
                _write_latest_state(out_dir, stage_index, emergency_path, None, row_for_checks, case_name)
            ...
            if float(config.checkpoint_interval_s) > 0.0 and float(system.global_time) - last_checkpoint_time >= float(config.checkpoint_interval_s) - 1.0e-12:
                ...
                _write_latest_state(out_dir, stage_index, checkpoint_path, None, row_for_checks, case_name)
        ...
        _write_latest_state(out_dir, stage_index, restart_path, summary_path, stage_rows[-1], case_name)
