"""Hydraulic-only V15 V71 pipe-fin radiator flow-path smoke run."""
...
DEFAULT_OUTPUT_PATH = Path(__file__).with_name("v15_v71_flow_path_smoke_result.json")
...
def run_smoke_case(
    *,
    ...
    result: Dict[str, Any] = {
        "case": "V15_V71_flow_path_hydraulic_only_smoke",
        "case_version": str(build["case_version"]),
        "axial_power_profile_name": str(build["axial_power_profile_name"]),
        "axial_power_profile_sum": float(np.sum(build["axial_power_profile"])),
        "axial_power_profile_heated_node_count": int(np.count_nonzero(build["axial_power_profile"] > 0.0)),
        "description": "Hydraulic-only V15 V71 smoke with center-0.30 m uniform core heating. Tube and fin emissivities are set to zero and no SystemManager thermal coupling step is run.",
