...
CORE_RING_SHARES_RAW = np.array([0.030269999, 0.166800001, 0.335999999, 0.4644], dtype=float)
...
def build_global_moderator_meshes(
    ...
    meshes = []
    radial_edges = np.linspace(float(inner_radius), float(outer_radius), int(n_rings) + 1)
...
def build_full_loop_core(
    ...
    *,
    ...
    mod_meshes = build_global_moderator_meshes(
        inner_radius=geom.r_moderator_outer,
