...
`run_prestart_cooldown.py` accepts `--initial-temperature` for uniform fluid and solid initialization. Its w0=8.12 external-heat tables treat the 1...360 CSV coordinate as orbital degrees and scale it to a `6552 s` physical period; the pre-start cases apply an external-heat absorptivity of `0.9`. `run_temperature_batch.py` runs the requested 15-case 300-340 K matrix: V14/V15 direct-radiator external heat plus V15 shield-only external heat, all with a `12000 s` limit, `260.55 K` coolant stop threshold, 1-s full-state records, and 60-s restart files.
