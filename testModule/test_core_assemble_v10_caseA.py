...
V10_CASE_VERSION = "v10_open_core_collector_ring"
V10_DEFAULT_INLET_TEMPERATURE_K = 753.330663091
...
def build_v10_case_a_system(
    inlet_temperature_k: float = V10_DEFAULT_INLET_TEMPERATURE_K,
    total_inlet_flow_kg_s: float = CASE_A_DESIGN_TOTAL_FLOW_KG_S,
    outlet_pressure_pa: float = V10_DEFAULT_OUTLET_PRESSURE_PA,
    pipe_n_nodes: int = 8,
    external_pipe_n_nodes: int = 5,
    connector_volume_m3: float = V10_DEFAULT_CONNECTOR_VOLUME_M3,
    connector_length_m: float = V10_DEFAULT_CONNECTOR_LENGTH_M,
    ...
    inlet_boundary = IncompressibleBoundaryVolume(
        name="V10_InletBoundary_FixedFlow",
        material=material,
        P=initial_p + 5000.0,
        T=initial_t,
    ...
    outlet_boundary = IncompressibleBoundaryVolume(
        name="V10_OutletBoundary_FixedPressure",
        material=material,
        P=initial_p,
        T=initial_t,
        flow_area=AREA_CORE_BRANCH * 3.0,
    ...
    header_to_cold_split = IncompressibleFluidVolume(
        name="V10_HeaderToColdReturnSplit",
        volume=float(connector_volume_m3),
        length=float(connector_length_m),
        flow_area=AREA_HEADER,
        hydraulic_diam=DH_HEADER,
    ...
    j_inlet = InletJunction(
        name="J_V10_InletBoundary_to_CoreInletConnector",
    ...
    j_outlet = FlowJunction(
        name="J_ColdReturnOutletMerge_to_OutletBoundary",
    ...
    for junc in all_juncs:
        ...
        if _is_core_channel_junction(name):
            ...
            value = macro_branch_flow
        elif name.startswith("J_Radiator"):
            value = total_flow
        else:
            value = total_flow
        junc.W = value
...
def reset_v10_design_flows(build: Dict[str, Any], *, preserve_ring_restart_flows: bool = False) -> None:
    ...
    for idx, junc in enumerate(net.junctions_obj):
        ...
        if preserve_ring_restart_flows and _is_preserved_ring_restart_junction(name):
            value = float(junc.W)
        elif name == "J_V10_InletBoundary_to_CoreInletConnector" or name == "J_ColdReturnOutletMerge_to_OutletBoundary":
            value = total_flow
...
def v10_flow_diagnostics(build: Dict[str, Any]) -> Dict[str, Any]:
    net = build["system"].fluid_solver
    ...
    return {
        ...
        "outlet_flow_kg_s": float(build["j_outlet"].W),
...
def v10_temperature_diagnostics(build: Dict[str, Any]) -> Dict[str, Any]:
    out = {
