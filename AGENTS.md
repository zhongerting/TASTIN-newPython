...
### 9.1 Git 与 GitHub 推送内容约束

用户要求提交并推送当前项目时，默认只允许以下内容进入 Git 提交和 GitHub：

- Python 代码：`*.py`、`*.pyi`。
- C++ 代码：`*.cpp`、`*.cc`、`*.cxx`、`*.h`、`*.hh`、`*.hpp`。
- Markdown 文档：`*.md`，包括本文件。
- ThermoCalc 最新生产扩展：仅 `ThermoCalc/te_solver.cp312-win_amd64.pyd`。

除非用户在当次任务中明确改变范围，否则不得提交或推送 CSV、JSON、NPZ、图片、日志、运行目录、输入快照、缓存、虚拟环境、构建目录、压缩包、旧版或备份 `.pyd`，也不得因排除上传而删除这些本地文件。旧 `.pyd` 已被 Git 跟踪时，应只从 Git 索引移除并保留本地文件；最新生产 `.pyd` 必须使用上述固定路径，不按时间戳猜测其他候选文件。

执行推送时必须先获取远端并处理分支差异，再精确暂存白名单文件，不得直接使用会把全部未跟踪产物纳入提交的宽泛暂存。提交前至少检查暂存文件类型、超大文件、嵌套 Git 仓库和 `git diff --cached --check`；推送后确认本地 `HEAD` 与 `origin/master` 一致。后续会话直接遵循本节，无需用户重复说明。
